...
/// A class to ping all IP numbers in network at a certain port.
/// Based on the library `NetworkAnalyzer`.
class ServerSearchHandler {
  ...
  static Stream<NetworkAddress> discover2(
    ...
    for (int i = start ?? 1; i < end ?? 100; ++i) {
      final host = '$subnet.$i';
      Future<Socket> f = _ping(host, port, timeout);
      futures.add(f);
      f.then((socket) {
        socket.destroy();
        ...
        if (e.osError == null || _errorCodes.contains(e.osError.errorCode)) {
          ...
          // Ignore: Error 23,24: Too many open files in system
        }
      });
    }
